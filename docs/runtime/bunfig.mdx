...
The path can be overridden with the `BUN_SYSTEM_CONFIG` environment variable, which must be an absolute path. Setting `BUN_SYSTEM_CONFIG` with a non-existent or malformed file is treated as an error (fail loudly), so policy typos are caught immediately.
